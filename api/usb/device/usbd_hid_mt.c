...
************************************************************************************************************/
#include "hw_config.h"
#if API_USBD_BIT_ENABLE && (USBD_TYPE_SUPPORT & BIT_ENUM(DEV_TYPE_HID)) && (USBD_HID_SUPPORT & BIT_ENUM(HID_TYPE_MT))
...
static uint8c_t mt_itf_desc_tab[] = {
...
**  Function
******************************************************************************************************/
error_t usbd_hid_mt_reset(uint8_t id)
{
    UNUSED_PARAMETER(id);
    return ERROR_SUCCESS;
}

error_t usbd_hid_mt_suspend(uint8_t id)
{
    UNUSED_PARAMETER(id);
    return ERROR_SUCCESS;
}
...
uint16_t usbd_hid_mt_get_itf_desc(uint8_t id, itf_ep_index_t* pindex, uint8_t* pdesc, uint16_t desc_len, uint16_t* pdesc_index)
{
	...
	len = sizeof(mt_itf_desc_tab);
	if (desc_len >= *pdesc_index + len) {
		memcpy(pdesc + *pdesc_index, mt_itf_desc_tab, len);
		usbd_assign_configuration_desc(id, DEV_TYPE_HID, HID_TYPE_MT, pindex, pdesc + *pdesc_index, len);

		rep_desc_len = get_hid_desc_map(TR_USBD, HID_TYPE_MT, NULL);
...
error_t usbd_hid_mt_out_process(uint8_t id, usbd_class_t* pclass)
{
    ...
    if((ERROR_SUCCESS == err) && usb_rxlen){
        logd("mt hid ep%d in%d:",pclass->endpout.addr, usb_rxlen);dumpd(usb_rxbuf,usb_rxlen);
    }
...
** Description:
*******************************************************************/
void usbd_hid_mt_process(uint8_t id, usbd_class_t *pclass, usbd_event_t evt, uint32_t val)
{
    switch(evt){
    case  USBD_EVENT_RESET:
        usbd_hid_mt_reset(id);
        break;
    case  USBD_EVENT_SUSPEND:
        usbd_hid_mt_suspend(id);
        break;
    case  USBD_EVENT_EP_OUT:
        usbd_hid_mt_out_process(id, pclass);
        break;
