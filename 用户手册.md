...
LietEMF 的设备驱动是在芯片上电后同一个函数统一初始化的，这个函数会在main函数前运行

LietEMF 关于支持任意种硬件架构, 支持在任意种的操作系统上可以移植

LiteEMF 为了灵活配置，用户可以在工程通过宏定义进行配置. 配置选项在 emf_config.h 中都有定义.  未来将借鉴 Linux 的 Kconfig 机制，使用该机制能够很方便地选择编译哪些功能。在终端，通过运行以下命令，可利用界面操作的方式对应用选项进行配置

LiteEMF 集成了 按键, led, rgb, 马达, 体感,手柄, 电池这些嵌入式常用的功能.还集成了nanopb(goolge protobuf),未来还将集成跟多的通用模块
