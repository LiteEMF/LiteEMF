...
typedef struct
{
    ...
    uint16_t           buf_size_max;   /**< Read/write index mask. Also used for size checking. */
    volatile uint8_t   fifo_stu;
...
static __INLINE uint16_t fifo_length(app_fifo_t * p_fifo)
{
    uint16_t tmp = p_fifo->read_pos;
	
    if(FIFO_FULL == p_fifo->fifo_stu){
    	return p_fifo->buf_size_max;
    }if(p_fifo->write_pos >= tmp){
    	return (p_fifo->write_pos - tmp);
    }else {
    	return (p_fifo->write_pos + p_fifo->buf_size_max - tmp);
    }
}

#define FIFO_EMPTY_LENGTH(p_fifo) ((p_fifo)->buf_size_max - fifo_length(p_fifo) + 1)
 ...
 *
 * @retval     ERROR_SUCCESS              If initialization was successful.
 ...
 *
 * @retval     ERROR_SUCCESS              If an element has been successfully added to the FIFO.
 ...
 *
 * @retval     ERROR_SUCCESS              If an element was returned.
 * @retval     ERROR_NOT_FOUND      If there are no more elements in the queue.
 */
 ...
 *
 * @retval     ERROR_SUCCESS              If an element was returned.
 * @retval     ERROR_NOT_FOUND      If there are no more elements in the queue, or the index was
 *                                      too large.
 */
 ...
 * @param[in]  p_fifo   Pointer to the FIFO.
 *
 * @retval     ERROR_SUCCESS              If the FIFO was flushed successfully.
 */
 ...
 *
 * @retval     ERROR_SUCCESS          If the procedure is successful. The actual number of bytes read might
 ...
 *
 * @retval     ERROR_SUCCESS       If the procedure is successful. The actual number of bytes written might
 ...
 * @retval     ERROR_NULL    If a NULL parameter was passed for a parameter that must not
 *                               be NULL.
 * @retval     ERROR_NO_MEM  If the FIFO is full.
 *
 */
